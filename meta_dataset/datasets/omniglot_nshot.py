...
class OmniglotNShot:
  ...
  def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
    ...
    self.datasets = {"train": self.x_train,
                     "test": self.x_test}  # original data cached
  ...
  def load_data_cache(self, data_pack):
    ...
    for sample in range(10):  # num of episodes
      ...
      for i in range(self.batchsz):  # one batch means one set
        ...
        # ex: if 5 way -> array([ 350, 1199,  143,  578,  818])
        ...
        for j, cur_class in enumerate(selected_cls):
          ...
          # if 5 shot & 15 query ->
          #   array([ 5,  3,  7, 13,  6, 11, 15, 17,  8,  1,  4, 18, 12, ... ])
        ...
        x_spt = np.array(x_spt).reshape(
            self.n_way * self.k_shot, 1, self.resize, self.resize)[perm]
        y_spt = np.array(y_spt).reshape(self.n_way * self.k_shot)[perm]
        perm = np.random.permutation(self.n_way * self.k_query)
        x_qry = np.array(x_qry).reshape(
            self.n_way * self.k_query, 1, self.resize, self.resize)[perm]
...
if __name__ == '__main__':
  ...
  db = OmniglotNShot('db/omniglot', batchsz=20, n_way=5,
                     k_shot=5, k_query=15, imgsz=64)
