...
class ImageNet(ImageDatasetClassSampler):
  ...
  def __init__(self, root, root_devkit=None, splits=None, download=False,
    ...
    self.maybe_download_devkit(root_devkit)
    if download:
      self.download_data()
  ...
  def maybe_download_devkit(self, root_devkit=None):
    if not os.path.isfile(self.devkit_metafile):
      archive_dict = ARCHIVE_DICT['devkit']
      if root_devkit is None:
        root_devkit = _splitexts(os.path.basename(archive_dict['url']))[0]
        root_devkit = path.join(self.root, root_devkit)
      if not path.isdir(root_devkit):
        download_and_extract_tar(archive_dict['url'], self.root,
                                 extract_root=self.root,
                                 md5=archive_dict['md5'])
      meta = parse_devkit(root_devkit)
